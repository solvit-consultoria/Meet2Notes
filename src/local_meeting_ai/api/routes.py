...
from local_meeting_ai.infrastructure.system_performance import system_performance_snapshot
...
def system_performance() -> dict[str, Any]:
    """Return lightweight, read-only process and system utilization metrics.

    GPU queries run in a throttled daemon thread; request handling only returns
    the last cached result and never waits for ``nvidia-smi``.
    """
    return {
